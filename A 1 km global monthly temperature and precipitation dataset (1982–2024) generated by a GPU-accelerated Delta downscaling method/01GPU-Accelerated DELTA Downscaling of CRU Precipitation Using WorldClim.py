...
# ================= Paths =================
CRU_NC_PATH = "G:/Geodata/Weather/Raster/CRUTS4.09/cru_ts4.09.1901.2024.pre.dat.nc"
WC_DIR = "G:/Geodata/Weather/Raster/WorldCLIMB/wc2.1_30s_prec"
OUT_DIR = "H:/WorldDeltaClim/pre/"
...
# Get and sort WorldClim precipitation files
...
def process_single_month(task_info):
    """
    Worker function: handles one month of interpolation, GPU multiplication, and writing.
    """
            ...
            with memfile.open() as mem_src, WarpedVRT(mem_src, **vrt_options) as vrt:
                with rasterio.open(tmp_filename, 'w', **out_meta) as dst:
                    for ji, window in wc_src.block_windows(1):
                        ...
                        # Core algorithm: precipitation downscaling uses multiplication
                        # PRE_downscaled = WorldClim_PRE * Anomaly_PRE
...
def main():
    print("1. Reading CRU precipitation data...")
    with rasterio.open(CRU_NC_PATH) as root_src:
        subdatasets = root_src.subdatasets
        pre_sds_path = next((s for s in subdatasets if s.endswith(':pre')),
                            f'netcdf:{CRU_NC_PATH}:pre')

    cru_src = rasterio.open(pre_sds_path)
    ...
    # Core algorithm: precipitation anomaly uses ratio
    # An_PRE = PRE_original / CRUClim_PRE
    # Handle division by zero and invalid values
    with np.errstate(divide='ignore', invalid='ignore'):
        all_anomalies = target_data / clim_tiled
        all_anomalies = np.where(clim_tiled == 0, 1.0, all_anomalies)
        all_anomalies = np.nan_to_num(all_anomalies, nan=1.0, posinf=1.0, neginf=1.0)

    del target_data, clim_tiled
    print("   -> Precomputation complete. Ready to dispatch GPU tasks.")
    # ----------------------------------------------------------------
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(futures),
                           desc="Downscaling progress"):
            try:
                future.result()
            except Exception as e:
                print(f"\nTask failed: {e}")

    print("\nPrecipitation downscaling completed successfully.")
